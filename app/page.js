"use client";
import { useState, useEffect, useRef } from "react";

// ═══════════════════════════════════════════════════════
// DATA LAYER — easy to swap for i18n later
// ═══════════════════════════════════════════════════════

const WHATSAPP_NUMBER = "77769262888";
const INSTAGRAM_URL = "https://www.instagram.com/serenity.clean";
const PRICE_PDF_URL = "serenity-price.pdf";

const LOGO_BLACK = "data:image/png;base64,";
const LOGO_WHITE = "data:image/png;base64,";

const BRAND = {
  name: "Serenity",
  tagline: "Химчистка в Астане",
};

const HERO = {
  title: "Профессиональный уход за вещами с\u00A0вниманием к\u00A0качеству и\u00A0деталям",
  subtitle:
    "Serenity — химчистка в Астане с аккуратным сервисом, понятной консультацией и бережным подходом к изделиям.",
  note: "Одежда, обувь, шторы, одеяла и подушки — с возможностью заранее уточнить стоимость, сроки и особенности обработки.",
  ctaPrimary: "Написать в WhatsApp",
  ctaSecondary: "Выбрать услуги",
};

const INTRO = {
  text: [
    "Serenity — современная химчистка в Астане для тех, кому важны аккуратность, понятный сервис и уверенность в результате.",
    "Вы можете заранее написать нам в WhatsApp, отправить фото изделия и уточнить стоимость, сроки и возможность обработки.",
  ],
};

const BENEFITS = [
  { icon: "🤲", title: "Бережный подход", text: "Каждое изделие требует внимательного отношения и правильно подобранной обработки." },
  { icon: "💬", title: "Понятная консультация", text: "Заранее объясняем важные нюансы, сроки и особенности обработки." },
  { icon: "✓", title: "Стабильное качество", text: "Для нас важны аккуратность, предсказуемость и общий уровень результата." },
  { icon: "📱", title: "Удобная связь", text: "Можно сразу написать в WhatsApp и уточнить всё по фото изделия." },
  { icon: "✦", title: "Аккуратная подача", text: "Для нас важен не только результат чистки, но и впечатление от сервиса в целом." },
  { icon: "◈", title: "Современный сервис", text: "Спокойный, понятный и аккуратно выстроенный клиентский опыт на каждом этапе." },
];

const CATEGORIES = [
  { icon: "🧥", title: "Верхняя одежда", desc: "Куртки, пальто, жилеты, ветровки" },
  { icon: "👟", title: "Обувь", desc: "Текстильная, кожаная, замшевая, деликатная, детская" },
  { icon: "☁️", title: "Одеяла и подушки", desc: "Изделия, требующие аккуратной и профессиональной обработки" },
  { icon: "🪟", title: "Шторы и тюль", desc: "Лёгкие, плотные и двусторонние изделия" },
  { icon: "🏠", title: "Домашний текстиль", desc: "Изделия для дома, которым нужен профессиональный уход" },
];

const SERVICE_GROUPS = [
  {
    title: "Куртки и верхняя одежда",
    note: "Точная стоимость зависит от типа, длины и особенностей изделия.",
    items: [
      { name: "Куртка зимняя", price: "от 7 000 ₸" },
      { name: "Пуховой жилет", price: "5 000 ₸" },
      { name: "Куртка-ветровка", price: "6 000 ₸" },
      { name: "Полупальто", price: "6 000 ₸" },
      { name: "Пальто демисезонное", price: "7 500 ₸" },
      { name: "Пальто зимнее", price: "10 000 ₸" },
    ],
  },
  {
    title: "Обувь",
    note: null,
    items: [
      { name: "Обувь текстиль / синтетика", price: "9 000 ₸" },
      { name: "Обувь кожа / замша / нубук", price: "10 000 ₸" },
      { name: "Обувь мех", price: "12 000 ₸" },
      { name: "Обувь деликатная", price: "12 000 ₸" },
      { name: "Детская обувь", price: "уточняется" },
    ],
  },
  {
    title: "Одеяла и подушки",
    note: null,
    items: [
      { name: "Одеяло полуторное", price: "5 000 ₸" },
      { name: "Одеяло двуспальное", price: "7 000 ₸" },
      { name: "Подушка синтепоновая", price: "4 000 ₸" },
      { name: "Подушка пуховая", price: "4 000 ₸" },
    ],
  },
  {
    title: "Шторы и тюль",
    note: "Для точного расчёта по шторам можно отправить фото и параметры в WhatsApp.",
    items: [
      { name: "Тюль", price: "от 2 500 ₸/кг" },
      { name: "Шторы обычные", price: "от 3 500 ₸/кг" },
      { name: "Шторы двусторонние", price: "уточняется" },
    ],
  },
];

const STEPS = [
  { num: "01", title: "Вы обращаетесь к нам", text: "Можно написать в WhatsApp, отправить фото изделия или выбрать нужные услуги прямо на сайте." },
  { num: "02", title: "Мы уточняем детали", text: "Подсказываем по стоимости, срокам и возможности обработки." },
  { num: "03", title: "Подбираем способ чистки", text: "Способ обработки определяется с учётом типа изделия, материала и особенностей ухода." },
  { num: "04", title: "Выполняем чистку и сушку", text: "Каждое изделие проходит обработку в соответствии с его характеристиками." },
  { num: "05", title: "Подготавливаем к выдаче", text: "После обработки изделие подготавливается к выдаче аккуратно и в надлежащем виде." },
];

const LOCATIONS = [
  { address: "Туран 22/1", hours: "Ежедневно: 09:00–20:00", gis: "https://go.2gis.com/5GxTc", wa: "77769262888" },
  { address: "Кайым Мухамедханова 4а, магазин Mitris", hours: "Ежедневно: 10:00–19:00", gis: "https://go.2gis.com/w8GVG", wa: "77714167908" },
  { address: "Алихан Бокейхан, 16/3", hours: "Ежедневно: 10:00–19:00", gis: "https://go.2gis.com/IbsFZ", wa: "77779262888" },
];

const FAQ_DATA = [
  { q: "Как узнать стоимость?", a: "Вы можете выбрать нужные позиции на сайте или просто написать нам в WhatsApp. По некоторым изделиям точная стоимость уточняется по типу, размеру или материалу." },
  { q: "Можно ли заранее отправить фото изделия?", a: "Да. По фото мы сможем предварительно сориентировать вас по стоимости, срокам и возможности обработки." },
  { q: "От чего зависит срок готовности?", a: "Срок зависит от типа изделия, материала, степени загрязнения и текущей загрузки. Точный срок сообщается при обращении." },
  { q: "Есть ли ускоренная обработка?", a: "Для части изделий такая возможность есть. Это уточняется индивидуально." },
  { q: "Можно ли обратиться на английском или казахском языке?", a: "Да, вы можете написать нам на русском, английском или казахском языке." },
  { q: "Где посмотреть полный прайс?", a: "Полный прайс доступен в отдельном PDF-файле на сайте." },
  { q: "Можно ли заранее понять, примете ли вы изделие в работу?", a: "Предварительно — да. Для этого лучше отправить фото изделия и, если есть, фото маркировки." },
];

// ═══════════════════════════════════════════════════════
// HELPERS
// ═══════════════════════════════════════════════════════

function waLink(message, number = WHATSAPP_NUMBER) {
  return `https://wa.me/${number}?text=${encodeURIComponent(message)}`;
}

const DEFAULT_WA_MSG =
  "Здравствуйте. Хочу узнать по чистке изделия. Подскажите, пожалуйста, по стоимости, срокам и возможности обработки.";

function buildServiceMsg(selected) {
  const items = selected.map((s) => `— ${s.name} (${s.price})`).join("\n");
  return `Здравствуйте. Хочу уточнить по следующим изделиям:\n\n${items}\n\nПодскажите, пожалуйста, по стоимости, срокам и возможности обработки.`;
}

// ═══════════════════════════════════════════════════════
// ANIMATION HOOK
// ═══════════════════════════════════════════════════════

function useReveal(threshold = 0.15) {
  const ref = useRef(null);
  const [visible, setVisible] = useState(false);
  useEffect(() => {
    const el = ref.current;
    if (!el) return;
    const obs = new IntersectionObserver(
      ([e]) => { if (e.isIntersecting) { setVisible(true); obs.disconnect(); } },
      { threshold }
    );
    obs.observe(el);
    return () => obs.disconnect();
  }, [threshold]);
  return [ref, visible];
}

function Reveal({ children, delay = 0, className = "" }) {
  const [ref, visible] = useReveal();
  return (
    <div
      ref={ref}
      className={className}
      style={{
        opacity: visible ? 1 : 0,
        transform: visible ? "translateY(0)" : "translateY(24px)",
        transition: `opacity 0.7s cubic-bezier(.22,1,.36,1) ${delay}s, transform 0.7s cubic-bezier(.22,1,.36,1) ${delay}s`,
      }}
    >
      {children}
    </div>
  );
}

// ═══════════════════════════════════════════════════════
// ICONS (inline SVG to avoid dependencies)
// ═══════════════════════════════════════════════════════

const WhatsAppIcon = ({ size = 20 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
  </svg>
);

const MapIcon = ({ size = 18 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/>
  </svg>
);

const ChevronDown = ({ size = 20 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <polyline points="6 9 12 15 18 9"/>
  </svg>
);

const InstagramIcon = ({ size = 20 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>
  </svg>
);

const CheckIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
    <polyline points="20 6 9 17 4 12"/>
  </svg>
);

// ═══════════════════════════════════════════════════════
// MAIN COMPONENT
// ═══════════════════════════════════════════════════════

export default function SerenityLanding() {
  const [selected, setSelected] = useState([]);
  const [openFaq, setOpenFaq] = useState(null);
  const [activeGroup, setActiveGroup] = useState(0);
  const servicesRef = useRef(null);

  const toggleItem = (item) => {
    setSelected((prev) => {
      const exists = prev.find((s) => s.name === item.name);
      return exists ? prev.filter((s) => s.name !== item.name) : [...prev, item];
    });
  };

  const isSelected = (name) => selected.some((s) => s.name === name);

  const scrollToServices = () => {
    servicesRef.current?.scrollIntoView({ behavior: "smooth" });
  };

  const sendSelectedToWA = () => {
    if (selected.length === 0) return;
    window.open(waLink(buildServiceMsg(selected)), "_blank");
  };

  return (
    <>


      {/* ════════ HEADER ════════ */}
      <header className="sr-header">
        <div className="sr-wrap sr-header-inner">
          <a href="#" className="sr-logo"><img src={LOGO_BLACK} alt="Serenity" className="sr-logo-img" /></a>
          <a href={waLink(DEFAULT_WA_MSG)} target="_blank" rel="noopener noreferrer" className="sr-header-wa">
            <WhatsAppIcon size={16} /> WhatsApp
          </a>
        </div>
      </header>

      {/* ════════ HERO ════════ */}
      <section className="sr-hero">
        <div className="sr-wrap">
          <Reveal>
            <div className="sr-hero-label">Химчистка в Астане</div>
          </Reveal>
          <Reveal delay={0.08}>
            <h1>{HERO.title}</h1>
          </Reveal>
          <Reveal delay={0.16}>
            <p className="sr-hero-sub">{HERO.subtitle}</p>
          </Reveal>
          <Reveal delay={0.24}>
            <div className="sr-hero-buttons">
              <a href={waLink(DEFAULT_WA_MSG)} target="_blank" rel="noopener noreferrer" className="sr-btn-primary">
                <WhatsAppIcon /> {HERO.ctaPrimary}
              </a>
              <button onClick={scrollToServices} className="sr-btn-secondary">
                ↓ {HERO.ctaSecondary}
              </button>
            </div>
          </Reveal>
          <Reveal delay={0.32}>
            <p className="sr-hero-note">{HERO.note}</p>
          </Reveal>
        </div>
      </section>

      {/* ════════ INTRO ════════ */}
      <section className="sr-intro">
        <div className="sr-wrap">
          <div className="sr-divider" style={{ marginBottom: 48 }} />
          <Reveal>
            {INTRO.text.map((t, i) => <p key={i}>{t}</p>)}
          </Reveal>
        </div>
      </section>

      {/* ════════ BENEFITS ════════ */}
      <section className="sr-section">
        <div className="sr-wrap">
          <Reveal>
            <div className="sr-section-header">
              <h2>Почему выбирают Serenity</h2>
              <p>Мы делаем ставку не только на результат обработки, но и на общее качество сервиса.</p>
            </div>
          </Reveal>
          <div className="sr-benefits-grid">
            {BENEFITS.map((b, i) => (
              <Reveal key={i} delay={i * 0.06}>
                <div className="sr-benefit-card">
                  <div className="sr-benefit-icon">{b.icon}</div>
                  <h3>{b.title}</h3>
                  <p>{b.text}</p>
                </div>
              </Reveal>
            ))}
          </div>
        </div>
      </section>

      {/* ════════ CATEGORIES ════════ */}
      <section className="sr-section" style={{ background: "#FFFFFF" }}>
        <div className="sr-wrap">
          <Reveal>
            <div className="sr-section-header">
              <h2>Что мы принимаем</h2>
              <p>Основные категории, с которыми мы работаем чаще всего.</p>
            </div>
          </Reveal>
          <Reveal delay={0.1}>
            <div className="sr-cat-grid">
              {CATEGORIES.map((c, i) => (
                <div className="sr-cat-card" key={i}>
                  <div className="sr-cat-icon">{c.icon}</div>
                  <h3>{c.title}</h3>
                  <p>{c.desc}</p>
                </div>
              ))}
            </div>
          </Reveal>
        </div>
      </section>

      {/* ════════ SERVICE SELECTOR ════════ */}
      <section className="sr-section-lg" ref={servicesRef} id="services">
        <div className="sr-wrap">
          <Reveal>
            <div className="sr-section-header">
              <h2>Выберите нужные изделия</h2>
              <p>Отметьте интересующие позиции, и мы подготовим сообщение для WhatsApp.</p>
            </div>
          </Reveal>

          <Reveal delay={0.1}>
            <div className="sr-services-tabs">
              {SERVICE_GROUPS.map((g, i) => (
                <button key={i} className={`sr-tab${activeGroup === i ? " active" : ""}`} onClick={() => setActiveGroup(i)}>
                  {g.title}
                </button>
              ))}
            </div>
          </Reveal>

          <Reveal delay={0.15}>
            <div className="sr-service-items">
              {SERVICE_GROUPS[activeGroup].items.map((item, i) => (
                <div
                  key={item.name}
                  className={`sr-service-item${isSelected(item.name) ? " checked" : ""}`}
                  onClick={() => toggleItem(item)}
                >
                  <div className="sr-service-check">
                    {isSelected(item.name) && <CheckIcon />}
                  </div>
                  <span className="sr-service-name">{item.name}</span>
                  <span className="sr-service-price">{item.price}</span>
                </div>
              ))}
            </div>
            {SERVICE_GROUPS[activeGroup].note && (
              <p className="sr-service-note">{SERVICE_GROUPS[activeGroup].note}</p>
            )}
          </Reveal>

          {selected.length > 0 && (
            <div className="sr-selected-bar">
              <div className="sr-wrap sr-selected-bar-inner">
                <span className="sr-selected-count">
                  Выбрано: <strong>{selected.length}</strong> {selected.length === 1 ? "позиция" : selected.length < 5 ? "позиции" : "позиций"}
                </span>
                <button className="sr-btn-wa-send" onClick={sendSelectedToWA}>
                  <WhatsAppIcon /> Отправить запрос в WhatsApp
                </button>
              </div>
            </div>
          )}
        </div>
      </section>

      {/* ════════ STEPS ════════ */}
      <section className="sr-section" style={{ background: "#FFFFFF" }}>
        <div className="sr-wrap">
          <Reveal>
            <div className="sr-section-header">
              <h2>Как проходит работа</h2>
            </div>
          </Reveal>
          <div className="sr-steps">
            {STEPS.map((s, i) => (
              <Reveal key={i} delay={i * 0.06}>
                <div className="sr-step">
                  <span className="sr-step-num">{s.num}</span>
                  <div>
                    <h3>{s.title}</h3>
                    <p>{s.text}</p>
                  </div>
                </div>
              </Reveal>
            ))}
          </div>
        </div>
      </section>

      {/* ════════ LOCATIONS ════════ */}
      <section className="sr-section">
        <div className="sr-wrap">
          <Reveal>
            <div className="sr-section-header">
              <h2>Наши пункты приёма</h2>
              <p>Выберите удобную для вас точку или напишите нам в WhatsApp.</p>
            </div>
          </Reveal>
          <div className="sr-locations-grid">
            {LOCATIONS.map((loc, i) => (
              <Reveal key={i} delay={i * 0.08}>
                <div className="sr-location-card">
                  <h3>{loc.address}</h3>
                  <p className="sr-location-hours">{loc.hours}</p>
                  <div className="sr-location-btns">
                    <a href={waLink(DEFAULT_WA_MSG, loc.wa)} target="_blank" rel="noopener noreferrer" className="sr-loc-btn sr-loc-btn-wa">
                      <WhatsAppIcon size={15} /> WhatsApp
                    </a>
                    <a href={loc.gis} target="_blank" rel="noopener noreferrer" className="sr-loc-btn sr-loc-btn-gis">
                      <MapIcon size={15} /> 2GIS
                    </a>
                  </div>
                </div>
              </Reveal>
            ))}
          </div>
        </div>
      </section>

      {/* ════════ WA BLOCK ════════ */}
      <section className="sr-section">
        <div className="sr-wrap">
          <Reveal>
            <div className="sr-wa-block">
              <h2>Удобнее всего — написать нам в&nbsp;WhatsApp</h2>
              <p>Если вы хотите заранее уточнить стоимость, сроки или возможность обработки, просто отправьте нам сообщение. Можно сразу приложить фото изделия — так мы быстрее сориентируем вас по деталям.</p>
              <a href={waLink(DEFAULT_WA_MSG)} target="_blank" rel="noopener noreferrer" className="sr-btn-wa-white">
                <WhatsAppIcon size={18} /> Написать в WhatsApp
              </a>
            </div>
          </Reveal>
        </div>
      </section>

      {/* ════════ PRICE PDF ════════ */}
      <section className="sr-section">
        <div className="sr-wrap">
          <Reveal>
            <div className="sr-price-block">
              <h2>Полный прайс</h2>
              <p>На сайте представлены самые частые категории. Полный прайс со всеми позициями можно открыть отдельно.</p>
              <a href={PRICE_PDF_URL} target="_blank" rel="noopener noreferrer" className="sr-btn-outline">
                📄 Смотреть полный прайс
              </a>
            </div>
          </Reveal>
        </div>
      </section>

      {/* ════════ FAQ ════════ */}
      <section className="sr-section" style={{ background: "#FFFFFF" }}>
        <div className="sr-wrap">
          <Reveal>
            <div className="sr-section-header">
              <h2>Частые вопросы</h2>
            </div>
          </Reveal>
          <div className="sr-faq-list">
            {FAQ_DATA.map((faq, i) => (
              <Reveal key={i} delay={i * 0.04}>
                <div className="sr-faq-item">
                  <button
                    className={`sr-faq-q${openFaq === i ? " open" : ""}`}
                    onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  >
                    {faq.q}
                    <ChevronDown />
                  </button>
                  <div
                    className="sr-faq-a"
                    style={{
                      maxHeight: openFaq === i ? 200 : 0,
                      opacity: openFaq === i ? 1 : 0,
                    }}
                  >
                    <div className="sr-faq-a-inner">{faq.a}</div>
                  </div>
                </div>
              </Reveal>
            ))}
          </div>
        </div>
      </section>

      {/* ════════ FINAL CTA ════════ */}
      <section className="sr-final-cta">
        <div className="sr-wrap">
          <Reveal>
            <h2>Отправьте запрос в WhatsApp</h2>
            <p>Подскажем по стоимости, срокам и возможности обработки.</p>
            <a href={waLink(DEFAULT_WA_MSG)} target="_blank" rel="noopener noreferrer" className="sr-btn-primary" style={{ maxWidth: 340, margin: "0 auto" }}>
              <WhatsAppIcon /> Написать в WhatsApp
            </a>
          </Reveal>
        </div>
      </section>

      {/* ════════ FOOTER ════════ */}
      <footer className="sr-footer">
        <div className="sr-wrap">
          <div className="sr-footer-grid">
            <div>
              <div className="sr-footer-brand"><img src={LOGO_BLACK} alt="Serenity" className="sr-footer-logo-img" /></div>
              <div className="sr-footer-tag">{BRAND.tagline}</div>
              <div className="sr-footer-socials">
                <a href={waLink(DEFAULT_WA_MSG)} target="_blank" rel="noopener noreferrer" className="sr-footer-social" title="WhatsApp">
                  <WhatsAppIcon size={18} />
                </a>
                <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer" className="sr-footer-social" title="Instagram">
                  <InstagramIcon size={18} />
                </a>
              </div>
            </div>
            <div>
              <h4>Пункты приёма</h4>
              {LOCATIONS.map((loc, i) => (
                <a key={i} href={loc.gis} target="_blank" rel="noopener noreferrer" className="sr-footer-link">
                  {loc.address}
                </a>
              ))}
            </div>
            <div>
              <h4>График работы</h4>
              <span className="sr-footer-link" style={{ cursor: "default" }}>Туран 22/1 — 09:00–20:00</span>
              <span className="sr-footer-link" style={{ cursor: "default" }}>Мухамедханова 4а — 10:00–19:00</span>
              <span className="sr-footer-link" style={{ cursor: "default" }}>Бокейхан, 16/3 — 10:00–19:00</span>
              <div style={{ marginTop: 14 }}>
                <a href={`tel:+${WHATSAPP_NUMBER}`} className="sr-footer-link" style={{ fontWeight: 500 }}>
                  +7 776 926 28 88
                </a>
              </div>
            </div>
          </div>
          <div className="sr-footer-bottom">
            © {new Date().getFullYear()} Serenity. Все права защищены.
          </div>
        </div>
      </footer>

      {/* ════════ FLOATING WA BUTTON ════════ */}
      <a
        href={waLink(DEFAULT_WA_MSG)}
        target="_blank"
        rel="noopener noreferrer"
        className="sr-float-wa"
        aria-label="Написать в WhatsApp"
      >
        <WhatsAppIcon size={28} />
      </a>
    </>
  );
}
